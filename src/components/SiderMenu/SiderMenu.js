...
export default class SiderMenu extends PureComponent {
  ...
  getNavMenuItems = (menusData) => {
    ...
    if (!(menusData instanceof Array)) {
      menusData = []; // eslint-disable-line
    }
    return menusData
  ...
  getSelectedMenuKeys = () => {
    ...
    return urlToList(pathname).map(
      itemPath => getMeunMatcheys(this.getFlatMenuKeys((!(this.props.menuData instanceof Array)) ?
        this.props.menuData : []), itemPath).pop());
  };
  // conversion Path
  // 转化路径
  conversionPath = (path) => {
  ...
  render() {
    ...
    if (!menuData || menuData.length === 0) menuData = undefined;
