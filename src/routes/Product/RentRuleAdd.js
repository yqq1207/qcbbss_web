...
@Form.create()
export default class RentRuleAdd extends PureComponent {
  state = {
    htmlContent: '',
    id: -1,
    type: 'add',
  ...
  componentDidMount() {
    ...
    if (!query) return null;
      ...
      const { id } = query;
      dispatch({
        type: 'rentRule/rentRuleDetailById',
        payload: {
          id,
        },
      });
  ...
  render() {
    ...
    const { orderInfoData } = this.props.rentRule;
    let htmlContent = '';
    let name = '';
    ...
    const updateInfo = this.props.rentRule.updateMessage;
    return (
      <PageHeaderLayout title="添加租赁规则模板">
